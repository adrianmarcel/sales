...
API REST em Spring Boot para venda de veículos construída sob o Clean Archtecture.
...
2. Suba a aplicação:
   ...
   ./gradlew bootRun
   ```
3. A API ficará disponível em `http://localhost:8998`.
...
- Construa a imagem Docker multi-stage:
  ```bash
  docker build -t sales:local .
  ```
- Rode localmente:
  ```bash
  docker run --rm -p 8998:8998 sales:local
  ```
